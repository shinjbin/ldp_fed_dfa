...
class Client(object):
    def __init__(self, client_id, alpha=10, device='cuda'):
        self.id = client_id
        self.device = device
    ...
    def local_train(self, B1, train_mode, tol=1e-4, train=True):
        ...
        if train:
            if train_mode == 'dfa':
                W1, W2, b1, b2 = self.nn.dfa_train(train_data=self.train_loader, B1=B1, tol=tol)
            elif train_mode == 'backprop':
                W1, W2, b1, b2 = self.nn.backprop_train(train_data=self.train_loader, tol=tol)
            else:
    ...
    def ldp(self, alpha, c, rho):
        ...
        ldp = LDP(W1_user=W1, W2_user=W2, b1_user=b1, b2_user=b2, alpha=alpha, c=c, rho=rho)

        ldp_W1, ldp_W2, ldp_b1, ldp_b2 = ldp.ordinal_cldp()
